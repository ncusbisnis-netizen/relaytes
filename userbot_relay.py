...
async def message_handler(event):
    ...
    if message.photo:
        ...
        # Ambil dari teks jika ada
        if text:
            six_digit = re.findall(r'(\d{6})', text)
            if six_digit:
                captcha_code = six_digit[0]
                logger.info(f"✅ Code from text: {captcha_code}")
    ...
    if not captcha_code and text:
        ...
        if six_digit:
            ...
            if any(kw in text.lower() for kw in keywords):
                is_captcha = True
                captcha_code = six_digit[0]
                logger.info(f"✅ Code from text+keyword: {captcha_code}")
    ...
    if not captcha_code and text:
        ...
        if lines and lines[0].strip().isdigit() and len(lines[0].strip()) == 6:
            ...
            logger.info(f"✅ Code from first line: {captcha_code}")
    
    # ===== JIKA CAPTCHA =====
    if is_captcha:
        logger.warning("🚫🚫🚫 CAPTCHA DARI BOT A!")
        
        # Jika captcha_code belum ada (foto tanpa teks), pakai OCR
        if not captcha_code and message.photo:
            logger.info("🔍 No text, trying OCR...")
        ...
        if captcha_code and len(captcha_code) == 6:
            ...
            logger.error("❌❌❌ Gagal dapat captcha code")
            ...
            if not bot_status['in_captcha'] and queue_length > 0:
                ...
                if request_id_bytes:
                    ...
                    if request_id in sent_requests:
                        ...
                        if time_diff < 30:
                            logger.info(f"⏳ Already sent {time_diff:.1f}s ago")
